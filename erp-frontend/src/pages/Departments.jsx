...
export default function Departments() {
  ...
  const del = async (id) => { if (!confirm('Delete department?')) return; await departmentsApi.delete(id); load() }
    ...
    <div className="page-enter">
      <div className="table-card">
        <div className="table-header">
          <span className="table-header-title">Departments</span>
          <button className="btn btn-primary ml-auto" onClick={openAdd}><Plus/>Add Department</button>
        </div>
        <div className="table-wrap">
          <table>
            <thead><tr><th>#</th><th>Name</th><th>Description</th><th>Employees</th><th></th></tr></thead>
            <tbody>
              {rows.length === 0
                ? <tr><td colSpan={5}><div className="empty-state"><Building2/><p>No departments</p></div></td></tr>
                : rows.map(r => (
                  <tr key={r.id}>
      ...
      {modal === 'form' && (
        <Modal title={editing?'Edit Department':'New Department'} onClose={closeModal}
          ...
          <div className="form-grid">
            <div className="form-field span2"><label className="form-label">Name *</label><input type="text" value={form.name} onChange={f('name')}/></div>
            <div className="form-field span2"><label className="form-label">Description</label><input type="text" value={form.description} onChange={f('description')}/></div>
          </div>
        </Modal>
      )}
    </div>
